...
async function generateIntoPool(sb, accessToken, profile, level, count) {
  ...
  // The subpath is the difference between a challenge for "freelancers" and one
  // for a copywriter, which is the whole reason it is asked.
  const subLabel = (paths.subpathsOf(path).find(s => s.slug === profile.subpath) || {}).label || null;
  ...
  const items = await askJSON(accessToken,
    'You are NoBossly, an execution coach who designs time-boxed challenges someone can finish around a job, pushing them toward real traction. Challenges are concrete, verifiable actions — never vague "work on your mindset" fluff. Avoid the words founder, entrepreneur and startup; say what they are actually doing.',
    `${brief ? brief.text + '\n\n' : ''}${pathDef ? 'PATH: ' + pathDef.label + ' — ' + pathDef.blurb + '\n' : ''}${subLabel ? 'SPECIFICALLY: ' + subLabel + ' — write for this, not for the path in general.\n\n' : (pathDef ? '\n' : '')}${answers ? 'THEIR OWN ANSWERS:\n' + answers + '\n\n' : ''}Their classification: business type "${profile.biz_type || 'unclassified'}", industry "${profile.biz_industry || 'unknown'}", customers "${profile.biz_segment || 'unknown'}", value prop "${profile.biz_value_prop || 'unknown'}". Their level: ${level} of 10 (1 = just starting, 10 = scaled).\n\nWrite ${Math.max(2, Math.min(5, count))} time-boxed challenges for someone on this path at this level. Use their answers to make each one land: the platform they picked, the hours and money they actually have, the deal breakers they named. Never write a challenge that breaks one of their deal breakers. Each must be concrete and verifiable, completable within the suggested days, and phrased so anyone else on the same path could also do it — so no references to their own brand name. Return a JSON array where each element has: title (short, action-oriented), description (1-2 sentences on the challenge and why it matters), emoji (single emoji), suggested_days (one of 30, 60, 90), xp_reward (integer 50-150).`,
    2000);
