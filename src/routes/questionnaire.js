...
// Fire-and-forget: notifies the user their questionnaire is done and ideas are ready.
function sendQuestionnaireCompleteEmail(userId, email, founderName) {
  ...
  fetch(`${base}/functions/v1/send-email`, {
    ...
    body: JSON.stringify({
      ...
      user_id: userId,
      to_email: email,
      to_name: founderName || 'Founder',
      data: {}
    })
  }).catch(e => console.error('[sendQuestionnaireCompleteEmail]', e.message));
}
...
router.post('/', async (req, res, next) => {
  try {
    ...
    // Congratulations email — ideas are generated by /ideas/generate which they
    // redirect to immediately after, so this email gives them something to come
    // back to if they close the tab.
    if (req.user?.email) {
      sendQuestionnaireCompleteEmail(req.user.id, req.user.email, q.founder_name);
    }
